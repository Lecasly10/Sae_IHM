...
class VueCrea {
	...
	init(form: TCreaForm): void {
        ...
        this.form.btnRetour.onclick = () => vueCrea.retour(); //retour à la page principale

        this.form.btnvalider.onclick = () => vueCrea.testValide(); //valide les infos de l'adhérent

        this.form.btnSupprimerAdhe.onclick = () => vueCrea.supprimer(); //supprime l'abonnement

        this.form.btnModifierAdhe.onclick = () => vueCrea.modifier(); //affiche la zone de modif l'abonnement quand on appuie sur modifier

        this.form.btnAjouterAdhe.onclick = () => vueCrea.affich(this.noLigne, 2); //affiche la zone d'ajout d'abonnement quand on appuie sur ajouter 

        this.form.btnValidModif.onclick = () => vueCrea.validerModif(); //valide la modification / ajout quand on appuie sur ✅​
        
        this.form.btnAnnulModif.onclick = () => vueCrea.annuler("annuler"); //annule la modification / ajout quand on appuie sur ❌​
	}
    ...
    modifier(): void { //affiche la zone de modif l'abonnement
        if (this.idSelect !== '') {
            vueCrea.affich(this.noLigne, 1);
            console.log("modifier");
        }
    }
}

let vueCrea = new VueCrea;
